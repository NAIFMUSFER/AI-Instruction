# -*- coding: utf-8 -*-
"""يكتب طبقة الجودة البصرية (المرحلة 9.1) في مواضعها بعد التفكيك (F-09).

كتلتان بحدّ معلَن بينهما، ولكلٍّ ملفّها اليوم:
1) كتلة كلاسيكية (تُستخرَج للاختبارات) في public/app/generated/pbr.js: مرآة
   المواصفة حرفياً + الدوالّ الحتمية pq* التي تُقارَن بايت-بايت مع بايثون +
   لوحة الإعدادات. أنماطها في public/app/styles/app.css وعلامتها في
   public/index.html.
2) جسر الوحدة في public/app/generated/pbr-bridge.js حيث يعيش THREE والمشهد:
   يطبّق الإعدادات على العرض الحيّ، قابلاً للعكس، ولا يلمس النموذج القانوني
   إطلاقاً. غياب THREE (كما في صندوق البناء) يعيد رفضاً مصنَّفاً
   PQ_THREE_UNAVAILABLE — لا انهيار.
"""
import io
import json
import os
...
JS_TARGET = os.path.join(APP.APP, "generated", "pbr.js")
BRIDGE_TARGET = os.path.join(APP.APP, "generated", "pbr-bridge.js")
CSS_TARGET = os.path.join(APP.APP, "styles", "app.css")
LATE_PATH = os.path.join(APP.APP, "late-bindings.js")
...
# أسماء يملكها مقطع لاحق (public/app/render/scene.js) فيقرؤها الجسر عبر سجلّ
# الربط المتأخّر: حافة أمامية مباشرة تفتح دورة في رسم الوحدات وتقلب ترتيب
# التقييم (F-09/F-11). القائمة مُعلَنة هنا، ويُتحقَّق أن كل اسم فيها مفتاح فعليّ
# في public/app/late-bindings.js — فلا اسم يُخترَع ولا رسم يُعاد استنباطه.
BRIDGE_LATE = ("camera", "lastBuilding", "model", "orbit", "setSun")

# خطّاف حلقة العرض (مبدّل المركِّب) يعيش في مقطع مكتوب بخطّ اليد لا في كتلة
# مولَّدة، فلا يكتبه هذا المولّد: يتحقّق من وجوده ويرفع خطأً صريحاً إن غاب.
LOOP_HOOK = "window.__ACS_PQ__&&window.__ACS_PQ__.composer"
...
def _late_keys():
    """مفاتيح سجلّ الربط المتأخّر كما يعلنها الملفّ نفسه — لا قائمة ثانية تنحرف."""
    src = _read(LATE_PATH)
    m = re.search(r"Object\.seal\(\{(.*?)\}\)", src, re.S)
    if not m:
...
def _late_bind(src, names):
    """يحوّل قراءة الاسم الحرّ إلى قراءة عبر السجلّ: X → __ACS_LATE.X.

    المسح لغويّ لا نصّيّ: يتخطّى النصوص والتعليقات، ولا يمسّ خاصيّة بعد نقطة،
    ولا مفتاح كائن ولا اختصاره داخل {}. الكتلة نفسها (tools/_pbr_bridge_block.js)
    تبقى مكتوبة بنطاق واحد كما كانت — التكييف مع نطاق الوحدة يجري هنا وحده.
    """
    missing = [n for n in names if n not in _late_keys()]
    if missing:
        _fail("late-bound name(s) %s are not keys of the registry in %s; "
              "declare them there (or drop them from BRIDGE_LATE) before "
              "regenerating." % (", ".join(missing), LATE_PATH))
    out = []
    i = 0
    n = len(src)
    prev = ""
    ...
    while i < n:
        ...
        if c in "'\"`":                                   # نصّ حرفيّ
            ...
            prev = c
            continue
        if c == "/" and src[i + 1:i + 2] == "/":           # تعليق سطر
            j = src.find("\n", i)
            j = n if j < 0 else j
            out.append(src[i:j])
            i = j
            continue
        if c == "/" and src[i + 1:i + 2] == "*":           # تعليق كتلة
            j = src.find("*/", i + 2)
            j = n if j < 0 else j + 2
            out.append(src[i:j])
            i = j
            continue
        if c.isalpha() or c in "_$":                       # معرّف
            j = i
            ...
            while k < n and src[k] in " \t\r\n":
                k += 1
            nxt = src[k:k + 1]
            inobj = bool(stack) and stack[-1] == "{"
            free = (word in names and prev != "."
                    and not (inobj and nxt == ":" and prev in "{,")
                    and not (inobj and nxt in "}," and prev in "{,"))
            if free:
                hits += 1
                out.append("__ACS_LATE." + word)
            else:
                out.append(word)
            i = j
            prev = word[-1]
        ...
        i += 1
    if not hits:
        _fail("none of the declared late-bound names %s occurs in "
              "tools/_pbr_bridge_block.js; BRIDGE_LATE is stale."
              % ", ".join(names))
    return "".join(out)


def _check_loop_hook():
    """خطّاف مبدّل المركِّب: مقيس لا مفترض، ولا يُكتب من هنا."""
    if LOOP_HOOK in APP.app_text():
        print("  present   composer switch in the render loop (hand-written "
              "module, not generated here)")
        return
    _fail("the composer switch is missing from the application modules.\n"
          "  expected the substring %r inside the render loop.\n"
          "  before F-09 this injector patched the loop inside "
          "public/index.html; the loop now lives in a hand-written module "
          "under public/app/, which no generator may edit. restore the hook "
          "there (see public/app/ui/workspace-ui-wiring.js) — the PBR "
          "composer is inert without it." % LOOP_HOOK)
...
def build():
    ...
    # الجسر: نصّه من tools/_pbr_bridge_block.js، مكيَّفاً لنطاق الوحدة بالربط
    # المتأخّر وحده. غلاف الوحدة (الاستيراد والتصدير) ملك tools/frontend_split.js
    # ولا يمسّه هذا المولّد.
    bridge = _late_bind(_read(os.path.join(HERE, "_pbr_bridge_block.js")),
                        BRIDGE_LATE)
    _splice(BRIDGE_TARGET, BRIDGE_SENTINEL_A, BRIDGE_SENTINEL_B,
            BRIDGE_SENTINEL_A + "\n" + bridge + "\n" + BRIDGE_SENTINEL_B)
    _check_loop_hook()
