...
The frontend is **one file**: `public/index.html` (~1.7 MB, ~29,650 lines). It is
served statically; there is no bundler, no framework and no build step for the
application code itself.

Structure inside that file:

- A plain (non-module) script that runs unconditionally: the login card, the
  panel UI, the API call path, and the error surface. This runs even if the 3D
  engine fails to load — that separation is deliberate.
- An `importmap` plus an `es-module-shims` fallback for browsers without native
  importmap support (iOS Safari < 16.4).
- A module script that imports Three.js from `/vendor` and builds the WebGL
  viewport.
- **Generated mirror blocks.** Several canonical Python layers have a
  line-by-line JavaScript mirror injected into the page by
  `tools/build_*_browser.py`. Each block is delimited by
  `/* ===== ACS <LAYER> LAYER (generated by tools/build_<x>_browser.py) ===== */`
  and is replaced wholesale on regeneration, so re-running an injector is
  idempotent. The `tests/phase*/parity` suites compare the Python layer and its
  browser mirror output character for character.
- A `window.ACS` global exposing the diagnostics the harnesses call:
  `renderDiagnostics()`, `verifyVisibleModel()`, `alignmentDiagnostics()`.

The backend base URL is a single constant in the page
(`CONFIGURED_BASE = "https://acs-engine.onrender.com"`), guarded by
`tools/check_api_base.py`, which also checks it against the CSP `connect-src`.

`public/vendor/` holds Three.js 0.160.0, es-module-shims 1.8.2 and pdfjs-dist
4.0.379. It is **not committed** — see §15 and §19.
