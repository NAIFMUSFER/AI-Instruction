...
This module never supplies defaults. It only distinguishes values that are not
needed to review horizontal Design Options/2D from values that must be explicit
before conceptual approval can become a Frozen Baseline for BIM/3D.
"""
...
def warehouse_vertical_stage_gate(model: dict, stage: str) -> dict:
    """Describe unresolved vertical values without inventing or normalising them."""
    ...
    missing = [key for key in _VERTICAL_FIELDS if not _resolved(model.get(key))]
...
def draft_geometry_admission(model: dict, issues: list[dict]) -> dict:
    """Split canonical findings for draft save/2D review; horizontal defects stay fatal."""
    gate = warehouse_vertical_stage_gate(model, "PLAN_DRAFT")
    deferred, blocking = [], []
    for issue in issues:
        path = issue.get("path") if isinstance(issue, dict) else None
        vertical = (gate["applicable"] and issue.get("code") == "DIMENSION_NOT_SPECIFIED"
...
def require_warehouse_vertical_for_downstream(model: dict, stage: str = "APPROVAL") -> None:
    """Fail closed at approval/BIM boundaries while preserving unknown values as unknown."""
    gate = warehouse_vertical_stage_gate(model, stage)
    if gate["blocking"]:
        raise PlanError(
            "DOWNSTREAM_GEOMETRY_NOT_SPECIFIED",
            "Warehouse vertical engineering values remain unresolved: " + ", ".join(gate["missing"]),
        )
