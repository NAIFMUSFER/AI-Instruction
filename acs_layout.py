...
def autofix(building, authority=AUTHORITY_PROPOSE, recorder=None):
    ...
    AUTHORITY_PROPOSE (الافتراضي): **لا يُقترح تغييرٌ إلا ويعود في التقرير**؛
        الحسابُ كلّه يجري على نسخة، ولا يُطبَّق منه شيء على النموذج المُمرَّر.
        استثناءٌ واحد مُعلَن، وهو ليس استثناءً من هذا المدخل بل من
        acs_engineering_authority.plan الذي يستدعيه: تطبيعات
        SAFE_NORMALIZATION تُطبَّق **على الكائن الممرَّر نفسه**، لأن البصمة
        المرجعية يجب أن تكون بصمة النموذج القانوني لا نموذجاً قبله (انظر
        عقد plan، وW1-B في plan_with_model).

        F-52 — تصحيح: كان هذا النصّ يقول «لا يُكتَب حرف في النموذج الهندسي»،
        وهو غير صحيح. القياس على ١٦٥ نموذجاً حقيقياً في المستودع: وضع
        الاقتراح يكتب في **١٠٨** منها. وثيقة plan كانت دقيقة وهذه لم تكن،
        فتناقضتا على المدخل الافتراضي — وهو أخطر موضع للتناقض، لأن من يقرأ
        هنا يظنّ نموذجه سليماً وهو ليس كذلك في ثلثَي الحالات.

        وحدود ما يُكتَب مقيسةٌ لا موعودة، وهذه هي كاملةً:
          wall_t · wall_h · floor_height   قيم نظام ناقصة تُملأ بالافتراضي
          meta.acs_provenance              سجلّ مصدر كل قيمة مُلئت
          rect                             تدوير إلى منزلتين ليس إلا
        ولا شيء غيرها. لا يُنقل جدار ولا يُغيَّر معرّف ولا تُضاف فتحة.
        tests/remediation/test_autofix_propose_boundary.py يقيس هذا الحدّ على
        المجموعة كاملة ويفشل إن اتّسع — فالوعد صار قيداً مفروضاً.
