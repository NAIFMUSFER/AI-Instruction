...
(function(){
  /* بعد F-09 لم تعد الكتلة محقونة في الصفحة بل صارت وحدة ES مستقلّة. الفحص
     صار أدقّ: العلامة موجودة مرّة واحدة في شيفرة التطبيق كلّها، والوحدة التي
     تحملها هي المولَّدة نفسها، وهي مستوردة فعلاً من public/app/main.js. */
  const APPSRC=require(_np.join(ROOT,'tests','phase3','lib_app_files.js'));
  const MOD='generated/workspace-ui.js';
  const B='/* ===== ACS WORKSPACE UI (generated by tools/build_workspace_ui.py) ===== */';
  const E='/* ===== END ACS WORKSPACE UI ===== */';
  chk('the generated workspace block appears exactly once in the whole application',
      APPSRC.countInApp(B)===1&&APPSRC.countInApp(E)===1,
      APPSRC.countInApp(B)+'/'+APPSRC.countInApp(E));
  chk('it lives in public/app/'+MOD+', and public/app/main.js imports it',
      APPSRC.moduleCarrying(B)===MOD&&APPSRC.order().indexOf(MOD)>=0);
  const page=APPSRC.mod(MOD);
