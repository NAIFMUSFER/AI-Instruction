...
# -------------------------------------------------------------- المرحلة 9.2
import acs_archdetail as _AD                                      # noqa: E402
ad = _open('acs_archdetail.py', encoding='utf-8').read()
adj = json.loads(_open('acs_archdetail.json', encoding='utf-8').read())
AB = ('/* ===== ACS ARCH DETAIL '
      '(generated by tools/build_archdetail_browser.py) ===== */')
AE = '/* ===== END ACS ARCH DETAIL ===== */'
ABR = '/* ===== ACS ARCH DETAIL BRIDGE (module scope) ===== */'
ABE = '/* ===== END ACS ARCH DETAIL BRIDGE ===== */'
chk('S-R1 طبقة التفصيل المعماري بايثون بلا تنفيذ ديناميكي', no_exec(ad))
chk('S-R2 كتلة التفصيل المولَّدة والجسر موجودان مرّة واحدة بالضبط',
    idx.count(AB) == 1 and idx.count(AE) == 1
    and idx.count(ABR) == 1 and idx.count(ABE) == 1)
adblock = idx[idx.index(AB):idx.index(AE)] \
    + idx[idx.index(ABR):idx.index(ABE)]
chk('S-R3 نسخة المتصفّح من طبقة التفصيل بلا eval/new Function',
    no_exec(adblock))
chk('S-R4 فحص التنفيذ الديناميكي غير عقيم على كتلة التفصيل',
    not no_exec(adblock + '\nvar x = eval("1+1");'))
chk('S-R5 الطبقة عرضية فقط في اتجاه المصبّ ولا سهم عائد',
    adj['presentation_only'] is True and adj['writes_to_model'] is False
    and adj['reverse_write_allowed'] is False
    and adj['reverse_arrow_exists'] is False
    and adj['mutates_engineering_model'] is False)
chk('S-R6 لا ترقية صنف عرضي ولا تغيير عدد الكائنات القانونية',
    adj['detail_class_promotion_allowed'] is False
    and all(b in adj['hard_stop_boundaries'] for b in (
        'PRESENTATION_TO_ENGINEERING_PROMOTION',
        'CANONICAL_DIMENSION_CHANGE_FOR_AESTHETICS',
        'INVENTED_BALCONY_GEOMETRY', 'INVENTED_PARKING_COUNT',
        'LED_TO_ELECTRICAL_CIRCUIT', 'RUNTIME_CDN_FETCH',
        'ARBITRARY_TEXTURE_URL', 'URL_BASED_GLTF_INJECTION',
        'EXECUTABLE_IMPORTED_ASSET',
        'CANONICAL_OBJECT_COUNT_CHANGE', 'SILENT_REQUEST_DROP')))
chk('S-R7 لا CDN وقت التشغيل في كتلة التفصيل ولا في جسرها',
    'http://' not in adblock and 'https://' not in adblock)
chk('S-R8 لا نسيج بعيد ولا مسار نظام ملفّات ولا GLTF من URL',
    adj['remote_texture_allowed'] is False
    and adj['user_filesystem_paths_allowed'] is False
    and adj['url_gltf_allowed'] is False
    and adj['executable_assets_allowed'] is False)
chk('S-R9 مفاتيح النموذج الأوّلي مرفوضة في تجاوزات خامات العرض',
    _AD.material('stone_beige', {'__proto__': 1})['valid'] is False
    and _AD.material('stone_beige',
                     {'constructor': 1})['valid'] is False)
chk('S-R10 نص معادٍ لا ينتج نيّة ولا يكسر المفسّر',
    _AD.interpret('<scr' + 'ipt>window.x=1</scr' + 'ipt>')['intents'] == []
    and _AD.interpret('javascript:alert(1)')['intents'] == []
    and _AD.interpret(None)['intents'] == [])
chk('S-R11 كائن معادٍ يُرفض من مكتبة العرض',
    _AD.object_recipe('../../etc/passwd')['valid'] is False
    and _AD.object_recipe('<img src=x>')['valid'] is False)
chk('S-R12 وصف المستخدم ليس إذناً هندسياً أبداً',
    _AD.interpret('بلكونات أكبر')['engineering_permission_granted'] is False
    and _AD.interpret('بلكونات أكبر')['intents'][0]['classification']
    == 'REQUIRES_ENGINEERING_CHANGE')
chk('S-R13 إنارة LED لا تُنشئ أي أثر كهربائي هندسي',
    (lambda L: L['valid'] and all(L['light'][k] is False for k in (
        'creates_electrical_circuit', 'creates_load',
        'creates_panel_assignment', 'creates_cable_route',
        'creates_mep_schedule_entry')))
    (_AD.led('facade_strip', {'represented': True})))
chk('S-R14 لوحة العرض المعماري بلا أي أداة تعديل هندسي',
    all(c in adj['panel_forbidden_controls']
        for c in ('engineering_dimension_edit', 'model_write',
                  'arbitrary_texture_url', 'mep_circuit_edit')))
chk('S-R15 لا ادّعاء كاذب: لا تتبّع مسارات ولا إضاءة عالمية ولا امتثال',
    all(adj[k] is False for k in (
        'photorealism_claimed', 'path_tracing_claimed',
        'ray_tracing_claimed', 'global_illumination_claimed',
        'code_compliance_claimed', 'parking_engineering_claimed',
        'landscape_design_claimed', 'lighting_design_claimed')))
chk('S-R16 الجسر يرفض إعداداً لا يصرّح writes_to_model=false',
    'cfg.writes_to_model!==false' in adblock)
chk('S-R17 التنويع حتمي البذرة ولا عشوائية وقت التشغيل',
    adj['variation']['deterministic'] is True
    and 'Math.random' not in adblock
    and _AD.variation('h', 'e', 'm') == _AD.variation('h', 'e', 'm'))
