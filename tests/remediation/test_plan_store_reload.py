...
def residential():
    return {
        ...
        "levels": [{"index": 0, "template": "ground"}],
        "floors": {"ground": {"rooms": [
            {"id": "majlis", "role": "living", "walls": "none", "rect": [0.0, 0.0, 8.0, 10.0]},
            {"id": "core", "role": "circulation", "walls": "none", "rect": [8.0, 0.0, 4.0, 10.0],
             "objects": [{"id": "lift_1", "kind": "elevator", "x": 1.0,
                           "z": 2.0, "w": 2.0, "d": 2.0, "h": 3.0}]},
            {"id": "bedroom", "role": "bedroom", "walls": "none", "rect": [12.0, 0.0, 8.0, 10.0]},
            {"id": "lounge", "role": "living", "walls": "none", "rect": [0.0, 10.0, 20.0, 10.0]},
        ]}},
    }
...
class PersistedWorkspaceReloadTests(unittest.TestCase):
    ...
    def saved_warehouse(self, *, approve=False):
        ...
        self.store.save_revision("p1", actor_id="owner-1", revision=first, expected_head=None)
        locked = ws.replace_semantic_locks(
            [element("racks", "rack_a", "storage"), element("docks", "dock_n1", "receiving")],
            expected_head=first.id, note="lock warehouse operating anchors")
        self.store.save_revision("p1", actor_id="owner-1", revision=locked, expected_head=first.id)
        approval = None
        if approve:
            approval = ws.approve(locked.id, expected_head=locked.id, actor_label="Engineer A",
                                  confirmed=True, acknowledge_concept_only=True)
            self.store.save_approval("p1", actor_id="owner-1", approval=approval, expected_head=locked.id)
    ...
    def test_reloaded_workspace_can_extend_history_and_persist_new_draft(self):
        ...
        saved = reopened.save_revision("p1", actor_id="owner-1", revision=draft, expected_head=locked.id)
        self.assertEqual(saved["revision_id"], draft.id)
        self.assertEqual(saved["number"], 3)
        self.assertEqual(reopened.project_state("p1", actor_id="owner-1")["head_revision_id"], draft.id)
    ...
    def test_residential_elevator_lock_survives_reload(self):
        ...
        self.store.save_revision("p1", actor_id="owner-1", revision=first, expected_head=None)
        locked = ws.replace_semantic_locks([element("objects", "lift_1", "core")],
                                           expected_head=first.id, note="lock elevator")
        self.store.save_revision("p1", actor_id="owner-1", revision=locked, expected_head=first.id)
        loaded = self.reload()
        changed = copy.deepcopy(locked.model)
    ...
    def test_later_draft_after_reload_does_not_replace_frozen_baseline(self):
        ...
        reopened.save_revision("p1", actor_id="owner-1", revision=draft, expected_head=locked.id)
