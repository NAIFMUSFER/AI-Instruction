...
/* 26 → 27 مع F-33 (ACS_INTEGRATION_ERROR)، ثم 28 مع F-50
   ...
   ثم 30 مع W2-E (ACS_UPSTREAM_NO_VISIBLE_OUTPUT)، ثم 31 مع حد الاستدعاءات. وقد عمل هذا الفحص فعلاً:
...
chk('acs_api_errors.CODES was read from the python source', CODES.length===31,
    CODES.length);
...
(function(){
  ...
  chk('the coverage check actually counted all 31 codes', cov.total===31, cov.total);
  CODES.forEach(code=>{
