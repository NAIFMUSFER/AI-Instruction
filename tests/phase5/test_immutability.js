...
(function(){
  ...
  chk('the browser layer exposes no generic setter',
      ['setModel','writeModel','applyPatch','setField','patchModel','mutateModel']
        .every(n=>eval('typeof '+n)==='undefined'));
  /* بعد F-09 صارت طبقة التأليف وحدة ES مستقلّة. الاقتطاع القديم من الصفحة صار
     يعطي نصّاً فارغاً فيمرّ الفحص بلا شيء يفحصه؛ فالمقاطع الآن تُنتزع من ملفّ
     الوحدة نفسه، ووجود كل علامة مؤكَّد صراحةً قبل الفحص. */
  const APPSRC=require(_np.join(ROOT,'tests','phase3','lib_app_files.js'));
  const AU_MOD='generated/authoring.js';
  const AU_OPEN='/* ===== ACS AUTHORING LAYER '
               +'(generated by tools/build_authoring_browser.py) ===== */';
  const AU_END='/* ===== END ACS AUTHORING LAYER ===== */';
  chk('the generated authoring layer appears exactly once in the whole application',
      APPSRC.countInApp(AU_OPEN)===1&&APPSRC.countInApp(AU_END)===1,
      APPSRC.countInApp(AU_OPEN)+'/'+APPSRC.countInApp(AU_END));
  chk('it lives in public/app/'+AU_MOD+', and public/app/main.js imports it',
      APPSRC.moduleCarrying(AU_OPEN)===AU_MOD
      &&APPSRC.order().indexOf(AU_MOD)>=0);
  chk('the developer API exposes no generic write entry point', (function(){
    const block=APPSRC.block(AU_MOD,AU_OPEN,AU_END);
    const at=block.indexOf('window.ACS');
    if(at<0) throw new Error('the authoring layer exposes no window.ACS surface '
      +'— the check would have inspected nothing');
    const api=block.slice(at);
    return api.length>200
      &&!/ACS\.(setModel|writeModel|applyPatch|patch|setField)\s*=/.test(api); })());
