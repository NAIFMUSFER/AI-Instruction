...
/* §11/§22 — كان هذا القسم داخل فرع الـDOM فلا يُنفَّذ إلّا في متصفّح حقيقي، مع
   أنه فحص مصدر بحت. أُخرِج من الفرع فصار يُنفَّذ في Node أيضاً — أي في السلسلة
   غير المتّصلة — وأُعيد توجيهه من نصّ الصفحة إلى شيفرة التطبيق نفسها بعد F-09. */
console.log('\n== §11/§22 — THE SHIPPED APPLICATION CODE ITSELF ==');
(function(){
  const APPSRC=require(_np.join(ROOT,'tests','phase3','lib_app_files.js'));
  const page=APPSRC.appText();
  const BRIDGE='/* ===== ACS PBR BRIDGE (module scope) ===== */';
  chk('the module-scope bridge is present exactly once',
      APPSRC.countInApp(BRIDGE)===1, APPSRC.countInApp(BRIDGE));
  chk('the bridge lives in the generated module that main.js imports',
      APPSRC.moduleCarrying(BRIDGE)==='generated/pbr-bridge.js'
      &&APPSRC.order().indexOf('generated/pbr-bridge.js')>=0);
  ...
  /* §22 — العبرة بجُمل التحميل الفعلية لا بذكر السياسة نثراً: تُستخرج كتلتا
     ...
     الاقتطاع الآن من ملفَّي الوحدتين، وكل علامة يُتحقَّق من وجودها لا يُفترض. */
  ...
  const qlayer=APPSRC.block('generated/pbr.js',_qb,_qe)
              +APPSRC.block('generated/pbr-bridge.js',_bb,_be);
