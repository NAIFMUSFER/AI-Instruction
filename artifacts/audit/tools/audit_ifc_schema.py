...
Usage: audit_ifc_schema.py CHECKOUT OUTPUT model1.json [model2.json ...]
...
for name in sys.argv[3:]:
    ...
    ifcopenshell.validate.validate(doc,logger)
    ...
    summary.append({'source':str(source),'export_valid':True,'schema':doc.schema,
                    'schema_errors':len(rows),'express_rules_evaluated':False,
