...
منتج منشور عند هذا التحديث هو C14: d7e72056ef718119e8e3c5785d1fcd17efa08070.
...
| AUDIT-C02b: غرف بلا وصول | أُصلح للنطاق المستطيل القابل للتقييم محلياً | test_room_access.py: 8 اختبارات ناجحة؛ رسم أبواب مشتركة ومناطق مفتوحة، مع كشف المكونات المعزولة حتى مع strict. الأغلفة المتداخلة ومداخل التحميل والأشكال غير المدعومة معلنة غير مكتملة؛ لا يشمل إخلاء الأدوار أو القبول الـ22. API يعلن PARTIAL عند نقص نطاق الوصول/النوى |
| AUDIT-C09a: glTF يستبدل المضلع بمستطيل | أُصلح في مصدر glTF؛ بقية C09 مفتوحة | test_polygon_gltf.py: 6 اختبارات؛ مساحة L الفعلية 20 م² وطرح فراغ النواة، جدران مائلة وفتحات، حفظ الأصل ورفض الحلقة المتقاطعة. SVG/IFC والمدقق والمستورد لم تُغلق بعد |
...
| AUDIT-C09c: مساحة المضلع ومعماره وطوبولوجيا الجدران | أُصلح في المصرّف المعماري ومرآته | 6 اختبارات Python و4 واجهة/تكافؤ؛ ستة جدران L ومساحة 20 م²، جدار قطري مشترك وباب صحيح، مساحة تقاطع فعلية وفراغ بلاطة؛ لا ترقية تلقائية لتعرض الفناء. SVG/IFC والمدقق والعلاقات/المسافات والمستورد ما زالت تحتاج الإكمال |
| AUDIT-C09d: حدود ومساحات المضلع في المستندات | أُصلح في SVG/PDF والقطاع والكمية | 6 اختبارات Python وحالتا تكافؤ؛ L=20 م²، ثلمة القطاع وفراغ النواة محفوظان، لا خطوط تقسيم حسابية داخلية. IFC والمدقق والمستورد ما زالت مفتوحة |
...
| AUDIT-C14: موضع مقطع IFC من النوع الخاطئ | أُصلح، مع حفظ إزاحته وتوافق الاستيراد | 3 اختبارات ناجحة؛ اختفت أخطاء Position التسعة من فحص IfcOpenShell. يبقى خطآ C15 منفصلين |
| AUDIT-C15: حقل أخير مفقود في الباب والنافذة IFC | أُصلح | اختباران ناجحان؛ لا أخطاء أنواع/عدد حقول في العينة وIFC المستودع. قواعد EXPRESS ما زالت تكشف C16/C17/C18 |
| AUDIT-C16/C17/C18: قواعد EXPRESS في IFC | مفتوحة ومثبتة مستقلاً | سجل تغيير ADDED بلا تاريخ تعديل؛ الغرف في containment خاطئ؛ جدران StandardCase بلا طبقات مواد. 7 أخطاء في العينة، دون علاقة بخطأ الحقول المصحح C15 |
...
آخر تشغيل كامل بعد C15: 133 ملف اختبار، 118 نجاح / 7 فشل / 8 غير متحققة.
