            ...
            for engine in engines:
                for width,height in [(390,844),(1440,1000)]:
                    ...
                    def export_model():
                        ...
                        payload=json.loads(Path(info.value.path()).read_text());(OUT/(tag+'-project.json')).write_text(json.dumps(payload,ensure_ascii=False,indent=2));h=history_from(payload);assert h,'Missing actual exported history';page.locator('#modal button[data-action="close-modal"]').first.click();return h
                    try:
                        ...
                        if args.baseline:
                            ...
                            fresh();f=page.locator('#brief-form');f.locator('[name="buildingAreaMin"]').fill('180');f.locator('[name="buildingAreaMax"]').fill('120');f.locator('[name="confirm"]').check();f.locator('button[type="submit"]').click();expect(page.locator('#modal-error')).to_be_visible();expect(page.locator('#modal-error')).to_contain_text('مساحة');check('invalid range explained inside the modal, not behind backdrop');original=page.locator('#brief-form .original-prompt');raw=original.text_content();case['originalReadback']={'expectedLength':len(prompt),'rawLength':len(raw),'rawEqual':raw==prompt};check('failed generation preserves exact original text bytes',raw==prompt);page.locator('#brief-form details summary').click();expect(original).to_be_visible();check('expanded original description preserves readable paragraphs',original.inner_text()==prompt)
                            box=page.locator('#modal-error');box.scroll_into_view_if_needed();check('error is actually in topmost hit-test layer',box.evaluate('(e)=>{const r=e.getBoundingClientRect();const p=document.elementFromPoint(r.x+r.width/2,r.y+r.height/2);return e===p||e.contains(p)}'))
