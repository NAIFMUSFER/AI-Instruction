...
function showModal(title, html, kicker = 'MASAR STUDIO') { lastFocus = document.activeElement; $('#modal-title').textContent = title; $('#modal-content').innerHTML = html; $('#modal-content').scrollTop = 0; $('#modal').scrollTop = 0; $('#modal-kicker').textContent = kicker; icons($('#modal')); if (!$('#modal').open)
    $('#modal').showModal(); }
...
بادل مع غرفة الطعام</div><div class="notice">المحلل المحلي حتمي ومحدود النطاق: إن لم يفهم الأمر يرفضه. الذكاء السحابي اختياري، وحتى عند استخدامه لا يملك صلاحية الاعتماد أو تجاوز الأقفال.</div>`);
        ...
        case 'regenerate-brief': {
            mutable();
            if(state.demo||!model().brief?.prompt)throw Error('ابدأ مشروعًا جديدًا بوصفك أولًا.');
            const original=model().brief.prompt,title=model().title;
            await persist(); await saveQueue;
            if(state.saveError)throw Error('تعذر حفظ المشروع الحالي؛ نزّل نسخة JSON قبل إنشاء توزيع مستقل.');
            const brief=understand(original);brief.title=title;
            confirmBrief(brief);
            const note=document.createElement('p');note.className='notice';note.id='rebuild-disclosure';
            note.textContent='المشروع السابق محفوظ دون تغيير. بعد مراجعة المدخلات وتأكيدها سننشئ مشروعًا مستقلًا من الوصف نفسه.';
            document.querySelector('#brief-form').prepend(note);
            break;
        }
        case 'render-center':
            await openRenderStudio({api,getModel:model,getRevision:()=>state.history.revisions[state.history.cursor].id,getCloudVersion:()=>state.versions.get(api.user.id+':'+model().id),isPending:()=>!!state.preview,isReadOnly:()=>state.readOnly,saveCloud,showModal,onSelect:selectRoom,reportError,isDemo:()=>state.demo});
            break;
        case 'account':
            await account();
            break;
...
async function handleForm(form) {
    ...
    switch (form.id) {
        ...
        case 'brief-form': {
            ...
            if(!data.has('confirm')) throw Error('أكد مراجعة المدخلات أولًا.');
...
// Native constraint-validation events do not trigger submit. Show them inside the modal,
