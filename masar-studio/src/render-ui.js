...
export async function openRenderStudio({api,getModel,getRevision,getCloudVersion,isPending,isReadOnly,saveCloud,showModal,onSelect,reportError,isDemo=()=>false}) {
    ...
    function viewStatus() {
        ...
        const state=viewer.viewState(),level=state.levelId?getModel().levels.find(l=>l.id===state.levelId):null;
        $('#render-view-state').textContent=`${level?level.name+' فقط — بقية الأدوار والموقع مخفية بصريًا':'كل الأدوار مع الموقع'} · ${state.visibleObjectIds.length} عناصر ظاهرة. لم يتغير التصميم.`;
    }
    function disableViewControls() {
        for(const id of ['#render-view-level','#render-view-angle','#render-view-fit'])$(id).disabled=true;
        if(alive())$('#render-view-state').textContent='تغيّرت إعدادات المشهد؛ اضغط «معاينة بالخامات» لتحديثه.';
    }
    for(const id of ['#render-finish','#render-quality','#render-room','#render-furniture'])$(id).addEventListener('change',disableViewControls);
    for(const id of ['#render-view-level','#render-view-angle','#render-view-fit'])$(id).addEventListener(id==='#render-view-fit'?'click':'change',execute(()=>{
        if(!viewer||$(id).disabled)return;
        viewer.setView({levelId:$('#render-view-level').value||null,preset:$('#render-view-angle').value});
        $('#render-selection').textContent='اختر مساحة ظاهرة لتحديدها في مخطط مسار.';viewStatus();
    }));
    ...
    $('#render-pbr').addEventListener('click',execute(async()=>{
        ...
        try {const s=snapshot(),v=await ensureViewer();if(!v||!alive())return;v.setScene(s);v.cutaway($('#render-cutaway').checked);
            ...
            $('#render-view-level').value='';$('#render-view-angle').value='iso';
            for(const id of ['#render-view-level','#render-view-angle','#render-view-fit'])$(id).disabled=false;
            viewStatus();
        } finally {previewBusy=false;if(alive())$('#render-pbr').disabled=false;}
    }));
    $('#render-cutaway').addEventListener('change',()=>{viewer?.cutaway($('#render-cutaway').checked);viewStatus();});
    ...
    $('#render-jobs').addEventListener('click',execute(async e=>{
        ...
        if(action==='glb'){downloads.invalidate();disableViewControls();if(job.stale||isPending()||job.revisionId!==revision()||job.projectId!==getModel().id)throw Error('لا يمكن عرض نسخة قديمة كأنها التصميم الحالي.');const v=await ensureViewer(),file=job.files.find(f=>f.name==='model.glb');const response=await fetch(file.url,{credentials:'same-origin',redirect:'error'});if(!response.ok)throw Error('تعذر تحميل المجسم.');await v?.loadGLB(await response.arrayBuffer());v?.cutaway($('#render-cutaway').checked);}
    }));
    await refresh();
}
