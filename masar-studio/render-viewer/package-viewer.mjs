...
const imports=["import * as THREE from 'three';", "import { OrbitControls } from 'three/addons/controls/OrbitControls.js';", "import { GLTFLoader } from 'three/addons/loaders/GLTFLoader.js';"].join('\n');
if(!source.includes(imports))throw Error('Unexpected viewer import contract');
await writeFile(path.join(dir,'../public/render-viewer.js'),source.replace(imports,"import {THREE,OrbitControls,GLTFLoader} from './render-engine.js';"));
