...
MAT = {
    ...
    # ---- مواد النظام العام للعناصر (بشر/مركبات/نبات/خرسانة) — مطابقة للواجهة ----
    "skin":       (0.78, 0.63, 0.48, 1.0, 0.00, 0.70),
    "fabric":     (0.23, 0.36, 0.55, 1.0, 0.00, 0.85),
    "paint_car":  (0.18, 0.31, 0.48, 1.0, 0.30, 0.35),
    "leaf":       (0.22, 0.55, 0.28, 1.0, 0.00, 0.80),
    "concrete_m": (0.62, 0.62, 0.64, 1.0, 0.00, 0.85),
}
...
# ---------------------------------------------------------------------------
# النظام العام للعناصر (room.objects) — منقول من الواجهة buildObjects/buildObject.
# عامّ لكل أنواع المباني (سكني/فندقي/عيادة/مكتب/مستودع) وغير مشروط بالصناعي.
# أي kind غير معروف يُبنى بصندوق بأبعاده واسمه (لا يُسقَط)، ويُسجَّل في تقرير التغطية.
# ---------------------------------------------------------------------------
OBJ_LIB = {
    # kind: (w, d, h, shape) — الأبعاد افتراضية يغلبها ما يذكره العميل
...
OBJ_MAT = {"person": "skin", "robot": "robot", "car": "paint_car", "truck": "paint_car",
           "forklift": "guard", "tree": "leaf", "palm": "leaf", "plant": "leaf",
           "column": "concrete_m", "stairs": "concrete_m", "elevator": "frame",
           "railing": "frame", "shelfobj": "deck", "pallet": "pallet", "panel": "tv",
           "sofa": "furn_soft", "bed": "furn_soft", "chair": "furn", "table": "furn", "box": "furn"}

_AR_OBJ_KEYS = sorted(OBJ_AR.keys(), key=len, reverse=True)
_EN_OBJ_KEYS = sorted(OBJ_LIB.keys(), key=len, reverse=True)
_AL_RANGE = "ء-ي"

def _obj_word_in(hay, key):
    """مطابقة بحدود الكلمة (تطابق frontend _wordIn): «رف» لا تطابق «أعرفه»."""
    k = re.escape(key)
    pat = ("(^|[^%s\\w])(?:ال|بال|لل|وال|و|ب|ل)?%s(?:ة|ه|ات|ين|ان|ي)?([^%s\\w]|$)"
           % (_AL_RANGE, k, _AL_RANGE))
    return re.search(pat, " " + hay + " ") is not None

def obj_kind(o):
    """يحلّ نوع العنصر: مباشر بالإنجليزية، ثم مرادف عربي بحدود الكلمة، ثم مفتاح إنجليزي.
    None = مجهول → يُبنى صندوقاً بأبعاده واسمه (لا يُسقَط)."""
    raw = str(o.get("kind") or o.get("name") or "").strip().lower()
    if raw in OBJ_LIB:
        return raw
    for k in _AR_OBJ_KEYS:
        if _obj_word_in(raw, k.lower()):
            return OBJ_AR[k]
    for k in _EN_OBJ_KEYS:
        if _obj_word_in(raw, k):
            return k
    return None

def build_object(bld, o, kind, cx, cz, base_y, fkey, room, idx):
    """يبني عنصراً واحداً في موضعه (إحداثيات عالمية) — منقول من frontend buildObject.
    بلا دوران (add_box محاذٍ للمحاور): الدوران تجميلي ولا يؤثّر على العدّ/الوجود."""
    dfl = OBJ_LIB.get(kind, (0.6, 0.6, 0.9, "box"))
    w = float(o.get("w") or dfl[0]); d = float(o.get("d") or dfl[1])
    h = float(o.get("h") or dfl[2]); shape = dfl[3]
    col = o.get("color")
    tag = "OBJ|%s|%s|%s%d" % (fkey, room, (o.get("name") or kind), idx)
    Mbase = OBJ_MAT.get(shape, "furn")

    def B(dx, dy, dz, ex, ey, ez, mat, c=None):
        bld.add_box(cx + dx, base_y + dy, cz + dz, ex, ey, ez, tinted(mat, c or col), tag)

    if shape == "person":
        B(0, h*0.22, 0, w*0.28, h*0.44, d*0.7, "skin", col or "#c8a07a")
        B(0, h*0.62, 0, w, h*0.34, d, "fabric", col or "#3b5b8c")
        B(0, h*0.90, 0, w*0.42, h*0.15, d*0.62, "skin", col or "#c8a07a")
    elif shape == "robot":
    ...
    elif shape in ("car", "truck"):
        body = col or ("#e8e8ea" if shape == "truck" else "#2f4f7a")
        B(0, h*0.33, 0, w, h*0.46, d, "paint_car", body)
        B(0, h*0.70, (-d*0.34 if shape == "truck" else 0), w*0.9, h*0.36,
          (d*0.24 if shape == "truck" else d*0.55), "window")
        for sx in (-1, 1):
            for sz in (-1, 1):
                B(sx*w*0.46, 0.33, sz*d*0.34, 0.22, 0.62, 0.62, "bumper")
    elif shape == "stairs":
        n = max(4, round(h/0.17)); rise = h/n; run = d/n
        for i in range(int(n)):
            B(0, rise*(i+0.5), -d/2+run*(i+0.5), w, rise, run, "concrete_m")
        for sx in (-1, 1):
            B(sx*(w/2+0.03), h*0.55, 0, 0.06, 0.06, d, "frame")
    elif shape == "elevator":
        ...
        for i in range(int(n)+1):
            B(-w/2+w*i/n, h*0.5, 0, 0.05, h, 0.05, "frame")
    elif shape in ("tree", "palm"):
        th = h*0.72 if shape == "palm" else h*0.42
        B(0, th*0.5, 0, w*0.16, th, d*0.16, "pallet", "#6b4a2a")
        if shape == "palm":
            import math as _m
            for i in range(6):
                a = i*_m.pi/3
                B(_m.cos(a)*w*0.32, th+0.18, _m.sin(a)*d*0.32, w*0.7, 0.10, d*0.2, "leaf")
        else:
            B(0, th+(h-th)*0.45, 0, w, (h-th)*0.9, d, "leaf")
    elif shape == "plant":
        B(0, 0.18, 0, w*0.55, 0.36, d*0.55, "furn", "#8a5a3a")
        B(0, h*0.62, 0, w, h*0.7, d, "leaf")
    elif shape == "sofa":
        B(0, 0.22, 0, w, 0.44, d, "fabric", col or "#5a6b8c")
        B(0, 0.55, -d*0.36, w, 0.62, d*0.28, "fabric", col or "#5a6b8c")
        for sx in (-1, 1):
            B(sx*(w/2-0.09), 0.52, 0, 0.18, 0.5, d, "fabric", col or "#5a6b8c")
    elif shape == "bed":
        B(0, 0.18, 0, w, 0.36, d, "furn", "#7a6a54")
        B(0, 0.44, 0, w*0.98, 0.20, d*0.94, "fabric", col or "#e8e4dc")
        ...
        for i in range(4):
            B(0, 0.15+i*(h-0.2)/4, 0, w, 0.05, d, "deck")
        for sx in (-1, 1):
            B(sx*(w/2-0.03), h*0.5, 0, 0.05, h, d, "frame")
    elif shape == "pallet":
        B(0, h*0.5, 0, w, h, d, "pallet")
    elif shape == "panel":
        hy = float(o["height"]) if o.get("height") is not None else (0.01 if kind == "rug" else 1.35)
        B(0, hy, 0, w, h, max(d, 0.04), Mbase)
    else:  # مجهول → صندوق بأبعاده
        B(0, h*0.5, 0, w, h, d, o.get("mat") or Mbase)

def build_objects(bld, room, fkey, base_y, acc=None):
    """يبني كل عناصر الغرفة مع التكرار (count/pitch/dir). لا يُسقَط أي عنصر:
    المجهول يُبنى صندوقاً ويُسجَّل. acc يجمّع تقرير التغطية (requested/generated/…)."""
    rx, rz, rw, rd = [float(v) for v in room["rect"]]
    nm = room.get("id", "room")
    for i, o in enumerate(room.get("objects") or []):
        kind = obj_kind(o)
        n = max(1, min(int(o.get("count") or 1), 200))
        pitch = float(o.get("pitch") or 1.2)
        dirn = "z" if o.get("dir") == "z" else "x"
        label = o.get("name") or o.get("kind") or kind or "?"
        if acc is not None:
            acc["requested"] += n
            acc["by_kind"][kind or ("generic:" + str(label))] = \
                acc["by_kind"].get(kind or ("generic:" + str(label)), 0) + n
            if kind is None:
                acc["generic_box"].append(str(label))
        for k in range(n):
            ox = float(o.get("x") if o.get("x") is not None else rw/2) + (pitch*k if dirn == "x" else 0)
            oz = float(o.get("z") if o.get("z") is not None else rd/2) + (pitch*k if dirn == "z" else 0)
            if ox > rw+2 or oz > rd+2:
                break
            build_object(bld, o, kind or "box", rx+ox, rz+oz,
                         base_y+float(o.get("y") or 0), fkey, nm, i*1000+k)
            if acc is not None:
                acc["generated"] += 1
...
def build_room(bld, room, fkey, base_y, defaults):
    ...
    # يرسم بشراً/مركبات/أثاثاً/مجهولاً؛ لا يُسقَط أي عنصر (مطابقة الواجهة).
    if room.get("objects"):
        build_objects(bld, room, fkey, base_y, _OBJ_ACC)
...
# requested/generated + generic_box (مُثِّل كصندوق عام) + excluded/added (من meta).
OBJECT_REPORT = {}

def compile_building(data, out_path):
    global _OBJ_ACC, OBJECT_REPORT
    _OBJ_ACC = {"requested": 0, "generated": 0, "generic_box": [], "unsupported": [], "by_kind": {}}
    bld = Builder()
    ...
    OBJECT_REPORT = {
        "requested": _OBJ_ACC["requested"],
        "generated": _OBJ_ACC["generated"],
        "generic_box": _OBJ_ACC["generic_box"],   # مُثِّل كصندوق عام (غير مُسقَط)
        "unsupported": _OBJ_ACC["unsupported"],    # لا شيء يُسقَط → عادةً فارغة
        "by_kind": _OBJ_ACC["by_kind"],
        "excluded": meta.get("excluded") or [],
        "auto_added": meta.get("added") or [],
    }
    if _OBJ_ACC["requested"]:
        print("[ACS] objects: requested=%d generated=%d generic_box=%d unsupported=%d"
              % (OBJECT_REPORT["requested"], OBJECT_REPORT["generated"],
                 len(OBJECT_REPORT["generic_box"]), len(OBJECT_REPORT["unsupported"])))
    return n, size
